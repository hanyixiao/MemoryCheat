...
// 更新数据
void CMemoryCheatDlg::UpTargetListData(int index, const TupleTargetLineData &info)
{
	// 设置描述
...
void CMemoryCheatDlg::OnBnClickedButtonSave()
{
	...
	// 修改内存数据,并更新列表框内容
	{
		...
		// 同时 修改列表框中的数据
		UpTargetListData(index, std::make_tuple(m_strDesEdit, dwAddr, m_strValueTypeEdit, m_strValueEdit));
	}
	UpdateData(FALSE);
}
...
void CMemoryCheatDlg::UpdateTargetListValueByAddress()
{
	...
	for (int i = 0; i < m_lstAddressTarget.GetItemCount(); ++i) {
		...
		// 更新本行内容:变化的只是 值
		UpTargetListData(i, std::make_tuple(get<0>(dt), dwAddr, valueType, value));
	}
}
